...
public class PageManager {
	...
	private int curdessertBlock; // 현재 페이가 속한 블럭
	private int rowCount; // 총 자료수
	// 다른 곳에서 활용하기에 public(어떤 페이지의 시작글번호,끝글번호,거꾸로 넘버링할 표기번호(게시글번호와는 다름),총페이지수)
	public int startRow, endRow, number, pageCount, curdessertPage;
	...
	public void setRowCount(int rowCount, int curdessertPage, int curdessertBlock) {
		this.curdessertPage = curdessertPage;
		this.rowCount = rowCount;
		this.curdessertBlock = curdessertBlock;
		number = rowCount - (curdessertPage - 1) * perPage;
		pageCount = getPageCount(rowCount);
		blockCount = getBlockCount(getPageCount(rowCount));// 총 몇개의 페이지 블럭으로
															// 나뉘었는가?
		startPage = getStartBlockPage(curdessertBlock, blockCount);// 이 블럭의 좌측 시작
		endPage = getEndBlockPage(curdessertBlock, blockCount, pageCount);
		if (endPage > pageCount)
			endPage = pageCount;
		startRow = getStartRow(curdessertPage, rowCount);
		endRow = getEndRow(curdessertPage, rowCount);
	...
	public String getPagenation(String boardJSPFile) {
		...
		if (rowCount > 0) {
			...
			if (startPage > perBlock) {
				int nextPage = curdessertPage - 1;
				// if(nextPage<1) nextPage=1;
				int nextBlock = curdessertBlock - 1;
			...
			for (int i = startPage; i <= endPage; i++) {
				buffer.append(
						"<a href='" + boardJSPFile + "?pageNum=" + i + "&blockNum=" + curdessertBlock + option + "' ");
