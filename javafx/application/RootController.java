...
public class RootController implements Initializable {
	...
	private Map<String, String> getMap() {
		...
		// getMax()에서 +1을 지우면 insert와 selectall의 serialno가 달라지는 에러가 없어짐.
	...
	void selectByConditions(ActionEvent event) {
		String conditions = JOptionPane.showInputDialog("WHERE 절을 포함한 조건을 입력해 주세요. ");
		try {
			table = TableViewFactory.getTable("SELECT * FROM HOMEBOOK "+conditions, ConnectionFactory.create());
			setTable();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public void initialize(URL location, ResourceBundle resources) {
		...
		comboAccounttitle.getItems().addAll("급여", "상여", "투자 수입", "쇼핑", "교통", "식대");
		...
		table.setOnMouseReleased(new EventHandler<MouseEvent>() {
			...
			public void handle(MouseEvent event) {
				...
				try {
					...
					if (selected.get("SECTION").equals("Revenue")) {
